...
@section('content')
    <div class="card">
        <div class="p-5 flex justify-between">
            <h6 class="text-lg text-gray-500 font-semibold">Edit Berita</h6>
            ...
            <form action="{{ route('berita.update', $berita->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <select name="category_id" id="category_id"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ old('category_id', $berita->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="content" id="content" rows="5"
                        class="py-3 px-4 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0"
                        placeholder="Masukkan Konten Berita" required>{{ old('content', $berita->content) }}</textarea>
                    @error('content')
                        <p class="text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
