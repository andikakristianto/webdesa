...
@section('content')
    <div class="card">
        <div class="p-5 flex justify-between">
            <h6 class=" text-lg text-gray-500 font-semibold">Tambah Data Masyarakat</h6>
            ...
            <form action="{{ route('masyarakat.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="nik" id="nik"
                        class="py-3 px-4 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0"
                        placeholder="Masukkan NIK Anda (Opsional)" value="{{ old('nik') }}">
                    @error('nik')
                        <p class="text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-6">
                    ...
                    <input type="text" name="kk" id="kk"
                        class="py-3 px-4 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0"
                        placeholder="Masukkan KK Anda (Opsional)" value="{{ old('kk') }}">
                    @error('kk')
                        <p class="text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="domisili" id="domisili"
                        class="py-3 px-4 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0"
                        placeholder="Masukan Domisili Anda" value="{{ old('domisili') }}">
                    @error('domisili')
                        <p class="text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
