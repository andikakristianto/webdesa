...
@section('title', 'Management Layanan Akte')

@section('content')
    <div class="card">
        <div class="card-title">
            <h1 class="p-4 font-bold text-center">Management Akta Kelahiran</h1>
        </div>
        <div class="card-body">
            <div class="card-body">
                <div class="relative overflow-x-auto">
                    <table id="AKTE" class="text-left w-full whitespace-nowrap text-sm text-gray-500">
                        <thead>
                            <tr class="text-sm">
                                ...
                                <th scope="col" class="p-4 font-semibold">KK</th>
...
@push('scripts')
    <script>
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if (row.status === "rejected") {
                                if (row.message === null) {
                                    return `<a href="/management/akte/comments/${row.id}" class="py-2 px-7 inline-flex items-center gap-x-2 text-base font-medium rounded-2xl border border-transparent bg-red-500 text-white hover:bg-red-600">Tambah</a>`;
                                } else {
                                    return data;
                                }
        ...
        function acceptRecord(id) {
            $.ajax({
                url: `{{ route('managementakte.pending', ':id') }}`.replace(':id', id),
                type: 'PUT',
                headers: {
        ...
        function doneRecord(id) {
            $.ajax({
                url: `{{ route('managementakte.done', ':id') }}`.replace(':id', id),
                type: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function rejectRecord(id) {
            $.ajax({
                url: `{{ route('managementakte.reject', ':id') }}`.replace(':id', id),
                type: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        function deleteRecord(id) {
            $.ajax({
                url: `{{ route('managementakte.destroy', ':id') }}`.replace(':id', id),
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
